Shader "CookbookShaders/3.7各向异性高光" {
	Properties {
		...
		_AnisoOffset ("AnisoOffset Offset", Range(-1,1)) = -0.2
	}
	SubShader {
		...
		inline fixed4 LightingAnisotropic (SurfaceAnisoOutput s, fixed3 lightDir, half3 viewDir, fixed atten)
		{
			...
			fixed4 c;
			c.rgb = ((s.Albedo * _LightColor0.rgb * NdotL) + (_LightColor0.rgb * _SpecularColor * spec)) * (atten * 2);
			c.a = 1.0;
			return c;
		}
